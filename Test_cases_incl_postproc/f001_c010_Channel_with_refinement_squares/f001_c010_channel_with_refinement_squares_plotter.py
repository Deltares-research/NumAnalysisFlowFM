...
# Split the dataset into 5 grids based on y-coordinates
y_min = np.min(y)
y_max = np.max(y)
y_range = y_max - y_min
grid_height = y_range / 5
...
for i in range(5):
    ...
    if i == 4:  # Last grid includes the maximum boundary
...
fig.suptitle('Water Depth (Constant) and Velocity (Constant) Distribution Across 5 Grid Levels', fontsize=14, fontweight='bold')
...
grid_labels = ['Grid 1', 'Grid 2', 'Grid 3', 'Grid 4', 'Grid 5']
