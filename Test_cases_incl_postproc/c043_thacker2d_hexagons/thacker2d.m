...
if init == 1;
    ...
    % Read x and y
    x          = nc_varget('radial2dhex_net.nc','NetNode_x');
    y          = nc_varget('radial2dhex_net.nc','NetNode_y');
    r          = sqrt(x.^2 + y.^2);
    ...
    % Write z
    nc_varput('radial2dhex_net.nc','NetNode_z',z);
    ...
    for j = 1:length(t);
        ...
        title(['t = ',num2str(t(j)/T*2,'%5.2f'),' \times T, after start (hexagons)'],'fontsize',fs);
        set(gca,'fontsize',fs);
        ...
        if printen == 1;
            if j == length(t)/2+1;
                ...
                print(figure(2),'-dpng','-r300','doc/thacker2dhexagonsT1.png');
                close all;
            end
            
            if j == length(t);
                legend('geometry','h  analytical','h  D-Flow FM','u  analytical','u  D-Flow FM','location','southeast');
                print(figure(2),'-dpng','-r300','doc/thacker2dhexagonsT2.png');
                close all;
            end
