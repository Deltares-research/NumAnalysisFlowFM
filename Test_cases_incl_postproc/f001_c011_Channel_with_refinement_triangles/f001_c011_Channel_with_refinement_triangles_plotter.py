...
# Option 1: Use face-centered data (current approach)
x_faces = data_vars['mesh2d_face_x'][:]  # x-coordinates of faces
y_faces = data_vars['mesh2d_face_y'][:]  # y-coordinates of faces
wd_faces = data_vars['mesh2d_waterdepth'][:]  # water depth (time, face)
s1_faces = data_vars['mesh2d_s1'][:]  # water level (time, face) 
bl_faces = data_vars['mesh2d_flowelem_bl'][:]  # bed level (face)
vel_faces = data_vars['mesh2d_ucmag'][:] if 'mesh2d_ucmag' in data_vars else None  # velocity magnitude (time, face)

# Option 2: Use nodal data by interpolating face values to nodes
x_nodes = data_vars['mesh2d_node_x'][:]  # x-coordinates of nodes
y_nodes = data_vars['mesh2d_node_y'][:]  # y-coordinates of nodes
face_nodes = data_vars['mesh2d_face_nodes'][:]  # face-to-node connectivity

# Interpolate face-centered water depth to nodes
def interpolate_face_to_node(face_values, face_nodes, n_nodes):
    """Interpolate face-centered values to nodes using area-weighted averaging"""
    node_values = np.zeros(n_nodes)
    node_weights = np.zeros(n_nodes)
    
    for face_idx, nodes in enumerate(face_nodes):
        # Handle triangular faces (3 nodes per face)
        for node_idx in nodes:
            if node_idx > 0:  # NetCDF indices might be 1-based
                node_idx_zero = int(node_idx - 1)  # Convert to 0-based
                if node_idx_zero < n_nodes:
                    node_values[node_idx_zero] += face_values[face_idx]
                    node_weights[node_idx_zero] += 1.0
...
# Interpolate water depth to nodes
wd_nodes_last = interpolate_face_to_node(wd_faces[time_idx, :], face_nodes, n_nodes)
s1_nodes_last = interpolate_face_to_node(s1_faces[time_idx, :], face_nodes, n_nodes)

print(f"\n=== FACE vs NODE COMPARISON ===")
print(f"Face-centered data: {len(x_faces)} points")
print(f"  Water depth range: {np.min(wd_faces[time_idx, :]):.6f} to {np.max(wd_faces[time_idx, :]):.6f}")
print(f"  Water depth std: {np.std(wd_faces[time_idx, :]):.6f}")

print(f"Node-centered data: {len(x_nodes)} points")
print(f"  Water depth range: {np.min(wd_nodes_last):.6f} to {np.max(wd_nodes_last):.6f}")
print(f"  Water depth std: {np.std(wd_nodes_last):.6f}")

# Choose which data to use for analysis
USE_NODAL_DATA = False  # Set to True to use nodal data, False for face data

if USE_NODAL_DATA:
    print(f"Using NODAL (interpolated) data for convergence analysis")
    x = x_nodes
    y = y_nodes  
    wd = np.array([wd_nodes_last])  # Make it 2D like original
    s1 = np.array([s1_nodes_last])
    # Note: For plotting we'll still use face data since it's native
    x_plot = x_faces
    y_plot = y_faces
    wd_plot = wd_faces
    vel_plot = vel_faces
else:
    print(f"Using FACE-CENTERED data for convergence analysis")
    x = x_faces
    y = y_faces
    wd = wd_faces
    s1 = s1_faces
...
bl = bl_faces  # Bed level stays the same
vel = vel_faces if vel_faces is not None else None
...
# For bed level, interpolate to nodes if using nodal data
if USE_NODAL_DATA:
    bl_nodes = interpolate_face_to_node(bl, face_nodes, n_nodes)
    bl_for_calc = bl_nodes
    print(f"Using interpolated bed level at nodes")
else:
    bl_for_calc = bl

# Check if water depth matches s1-bl (only if dimensions match)
if USE_NODAL_DATA:
    depth_diff = np.abs(wd[0, :] - (s1[0, :] - bl_for_calc))
else:
    depth_diff = np.abs(wd - (s1_faces - bl))
print(f"Max difference between wd and (s1-bl): {np.max(depth_diff):.6f} m")

# Velocity analysis (always use face data since velocity is naturally face-centered)
if vel_faces is not None:
    print(f"\nVelocity magnitude (ucmag) analysis:")
    print(f"  Shape: {vel_faces.shape}")
    print(f"  Range: {np.min(vel_faces):.6f} to {np.max(vel_faces):.6f} m/s")
    print(f"  Mean: {np.mean(vel_faces):.6f} m/s")
    print(f"  Std: {np.std(vel_faces):.6f} m/s")
    print(f"  Non-zero values: {np.sum(vel_faces > 1e-10)}")
    print(f"  Zero/near-zero values: {np.sum(vel_faces <= 1e-10)}")
    ...
    vel_diff = np.abs(vel_faces - vel_calculated)
    print(f"  Max difference between ucmag and sqrt(ucx^2+ucy^2): {np.max(vel_diff):.6e}")
    
    # For final time step
    print(f"\nVelocity at time step {time_idx}:")
    print(f"  ucmag range: {np.min(vel_faces[time_idx, :]):.6f} to {np.max(vel_faces[time_idx, :]):.6f} m/s")
    ...
    high_vel_mask = vel_faces[time_idx, :] > 10.0  # velocities > 10 m/s
...
wd_current = wd[time_idx, :] if wd.ndim > 1 else wd
print(f"\nActual water depth values for time step {time_idx}:")
print(f"  Mean: {np.mean(wd_current):.3f} m")
print(f"  Std:  {np.std(wd_current):.3f} m")
print(f"  Min:  {np.min(wd_current):.3f} m")
print(f"  Max:  {np.max(wd_current):.3f} m")

# Check if the data is actually varying spatially or if there's something else
s1_current = s1[time_idx, :] if s1.ndim > 1 else s1
print(f"\nWater level (s1) for time step {time_idx}:")
print(f"  Mean: {np.mean(s1_current):.3f} m")
print(f"  Std:  {np.std(s1_current):.3f} m")
print(f"  Min:  {np.min(s1_current):.3f} m")
print(f"  Max:  {np.max(s1_current):.3f} m")

print(f"\nBed level (bl):")
print(f"  Mean: {np.mean(bl_for_calc):.3f} m")
print(f"  Std:  {np.std(bl_for_calc):.3f} m")
print(f"  Min:  {np.min(bl_for_calc):.3f} m")
print(f"  Max:  {np.max(bl_for_calc):.3f} m")
print("===================\n")

# Split the dataset into 3 grids based on y-coordinates
y_min = np.min(y)
y_max = np.max(y)
y_range = y_max - y_min
grid_height = y_range / 3

# Create indices for each grid
grid_indices = []
for i in range(3):
    y_lower = y_min + i * grid_height
    y_upper = y_min + (i + 1) * grid_height
    if i == 2:  # Last grid includes the maximum boundary
        mask = (y >= y_lower) & (y <= y_upper)
    else:
        mask = (y >= y_lower) & (y < y_upper)
    grid_indices.append(mask)

print(f"Y range: {y_min:.3f} to {y_max:.3f}")
print(f"Grid height: {grid_height:.3f}")
for i, mask in enumerate(grid_indices):
    print(f"Grid {i+1}: {np.sum(mask)} points")

# For plotting, always use face-centered data for better visualization
y_plot_min = np.min(y_plot)
y_plot_max = np.max(y_plot)
y_plot_range = y_plot_max - y_plot_min
grid_plot_height = y_plot_range / 3

# Create indices for plotting
grid_plot_indices = []
for i in range(3):
    y_lower = y_plot_min + i * grid_plot_height
    y_upper = y_plot_min + (i + 1) * grid_plot_height
    if i == 2:  # Last grid includes the maximum boundary
        mask = (y_plot >= y_lower) & (y_plot <= y_upper)
    else:
        mask = (y_plot >= y_lower) & (y_plot < y_upper)
    grid_plot_indices.append(mask)

# Create figure with 3 rows and 2 columns
fig, axes = plt.subplots(3, 2, figsize=(12, 10))
fig.suptitle('Water Depth (Constant) and Velocity (Constant) Distribution Across 3 Grid Levels', fontsize=14, fontweight='bold')

for grid_num in range(3):
    mask = grid_plot_indices[grid_num]
    
    if np.sum(mask) == 0:
        print(f"Warning: Grid {grid_num+1} has no data points")
        continue
    
    # Extract data for this grid (use plotting data)
    x_grid = x_plot[mask]
    y_grid = y_plot[mask]
    wd_grid = wd_plot[time_idx, mask]
    ...
    scatter_wd = ax_wd.scatter(x_grid, y_grid, c=wd_grid, cmap='Blues', s=50, alpha=0.8, 
                              vmin=3.89, vmax=3.91)  # Fixed scale to show the small variations
    ...
    if vel_plot is not None:
        vel_grid = vel_plot[time_idx, mask]
        ...
        if vel_std < 0.01 * vel_mean:  # If velocity is nearly constant (1% variation)
            ...
            scatter_vel = ax_vel.scatter(x_grid, y_grid, c=vel_grid, cmap='Reds', s=50, alpha=0.8,
                                       vmin=vel_mean*0.95, vmax=vel_mean*1.05)  # Tight scale around mean
            ...
            if len(x_grid) >= 3:
                try:
                    if HAS_SCIPY:
                        ...
                        # Create filled contour plot for velocity with proper limits
                        contour_vel = ax_vel.contourf(xi_mesh, yi_mesh, vel_interp, levels=20, cmap='Reds', alpha=0.8,
                                                     vmin=np.min(vel_grid)*0.9, vmax=np.max(vel_grid)*1.1)
                    else:
                        # Use triangular contour plot with proper limits
...
for grid_num in range(3):
    ...
    # Extract data for this grid (use convergence analysis data)
    x_grid = x[mask]
    wd_grid = wd[time_idx, mask] if wd.ndim > 1 else wd[mask]
...
wd_current = wd[time_idx, :] if wd.ndim > 1 else wd
y_min_plot = min(np.min(wd_current), analytical_depth) * 0.95
y_max_plot = max(np.max(wd_current), analytical_depth) * 1.05
...
wd_current = wd[time_idx, :] if wd.ndim > 1 else wd
print(f"Numerical water depth range: {np.min(wd_current):.3f} to {np.max(wd_current):.3f} m")
print(f"Difference from analytical: {abs(np.mean(wd_current) - analytical_depth):.6f} m")
...
for grid_num in range(3):
    ...
    l2_norm = np.sqrt(np.sum(differences**2) / n_points)
    
    grid_levels.append(grid_num + 1)
    l2_norms.append(l2_norm)
    grid_sizes.append(n_points)
    
    print(f"Grid {grid_num+1}: {n_points} points, L2 norm = {l2_norm:.6e} m")
...
ax_convergence.set_xlabel('Number of Grid Points')
...
data_type = "nodal" if USE_NODAL_DATA else "face-centered"
print(f"Using {data_type} data for convergence analysis")
for i, (level, size, norm) in enumerate(zip(grid_levels, grid_sizes, l2_norms)):
    print(f"Grid {level}: {size:4d} points, L2 norm = {norm:.6e} m")
    if i > 0:
