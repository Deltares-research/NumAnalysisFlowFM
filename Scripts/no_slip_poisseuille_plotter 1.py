...
PATH = 'C:/Users/star_kj/OneDrive - Stichting Deltares/Documents//04_Software/D-HYDRO/Validation_cases/c012_poiseuille_noslip/' #os.getcwd() # Ensure that this file is present in the test folder that contains ...\dflowfmoutput
OUTPUT_FOLDER = "dflowfmoutput" # The folder where the output files are stored
MDU_FILE_NAME = "poiseuillenoslip"
MAP_FILE_NAME = "poiseuillenoslip_map.nc"
...
kappa = 0.41
g = 9.81      # [m/s^2]
dx = 25 # half a cell size in x-direction
...
x_section = np.mean(x)+dx  # or set to desired x value
cross_idxs = find_indexes(x, x_section)
x_cross = x[cross_idxs]
y_cross = y[cross_idxs]
u_num = data_vars['mesh2d_ucx'][-1, cross_idxs]  # t=0, adjust index as needed

# --- Analytical solution ---
def analytical_velocity(y, g,  B, L, nu_h):

    return g * DH / (8*nu_h*L) * ( 4*y**2 - B**2)


# For a cross-section, y goes from -B/2 to B/2
y_analytical = y_cross - np.mean(y_cross)  # center at 0
u_analytical = analytical_velocity(y_analytical, g,  B, L, nu_h)
a = 1+1
# --- Plot ---
import matplotlib.pyplot as plt
